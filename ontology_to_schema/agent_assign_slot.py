...
class CustomOutputSchema(BaseIOSchema):
    ...
    The output will be a list of booleans, with each boolean indicating whether the
    corresponding query from the user is true (1) or false (0).
    """
    ...
    response: str = Field(
        ...
        description=(
            ...
            "field corresponding to a query from the user, and the value (boolean) "
            "indicating whether the query is true (1) or false (0)."
        ),
    )
...
class CustomInputSchema(BaseIOSchema):
    ...
    The input will be a string representing a list of tuples, where each tuple contains
    two elements: the first element is the class name, and the second element is the
    attribute name. Each tuple corresponds to a query from the user.
    """

    query: str = Field(
        ...,
        description=(
            "A string representing a list tuples, each consisting of a class name and an attribute name, "
...
class AgentAssignSlot(Agent):
    ...
    def query(self, cls_name: List[str], slot_name: List[str]) -> Dict[str, bool]:
